...
namespace BEPUutilities
{
    ...
    public static class Toolbox
    {
        ...
        /// </summary>
        public static FP BigEpsilon = F64.C1 / new FP(100000);

        /// <summary>
        /// Tolerance value. Defaults to 1e-7f.
        /// </summary>
        public static FP Epsilon = F64.C1 / new FP(10000000);

        /// <summary>
        /// Represents an invalid Vector3.
        /// </summary>
        public static readonly FPVector3 NoVector = new FPVector3(-FP.MaxValue, -FP.MaxValue, -FP.MaxValue);
        ...
        public static bool FindRayTriangleIntersection(ref FPRay ray, FP maximumLength, ref FPVector3 a, ref FPVector3 b, ref FPVector3 c, out bool hitClockwise, out FPRayHit hit)
        {
            hitClockwise = false;
            hit = new FPRayHit();
            FPVector3 ab, ac;
            ...
            FP d;
            FPVector3.Dot(ref ray.Direction, ref hit.Normal, out d);
            d = -d;

            hitClockwise = d >= F64.C0;

            FPVector3 ap;
            FPVector3.Subtract(ref ray.Position, ref a, out ap);
            ...
            FPVector3.Add(ref ray.Position, ref hit.Location, out hit.Location);

            // Compute barycentric coordinates
            FPVector3.Subtract(ref hit.Location, ref a, out ap);
            FP ABdotAB, ABdotAC, ABdotAP;
            FP ACdotAC, ACdotAP;
            ...
            FP denom = F64.C1 / (ABdotAB * ACdotAC - ABdotAC * ABdotAC);
            FP u = (ACdotAC * ABdotAP - ABdotAC * ACdotAP) * denom;
            FP v = (ABdotAB * ACdotAP - ABdotAC * ABdotAP) * denom;
        ...
        /// <param name="hit">Hit data of the ray, if any</param>
        /// <returns>Whether or not the ray and triangle intersect.</returns>
        public static bool FindRayTriangleIntersection(ref FPRay ray, FP maximumLength, TriangleSidedness sidedness, ref FPVector3 a, ref FPVector3 b, ref FPVector3 c, out FPRayHit hit)
        {
            hit = new FPRayHit();
            FPVector3 ab, ac;
            FPVector3.Subtract(ref b, ref a, out ab);
        ...
        /// <param name="q">Intersection point.</param>
        /// <returns>Whether or not the segment intersects the plane.</returns>
        public static bool GetSegmentPlaneIntersection(FPVector3 a, FPVector3 b, FPPlane p, out FP t, out FPVector3 q)
        {
            return GetLinePlaneIntersection(ref a, ref b, ref p, out t, out q) && t >= F64.C0 && t <= F64.C1;
        }
        ...
        public static bool GetLinePlaneIntersection(ref FPVector3 a, ref FPVector3 b, ref FPPlane p, out FP t, out FPVector3 q)
        {
            FPVector3 ab;
            FPVector3.Subtract(ref b, ref a, out ab);
            FP denominator;
        ...
        /// <param name="q">Intersection point.</param>
        /// <returns>Whether or not the line intersects the plane.  If false, the line is parallel to the plane's surface.</returns>
        public static bool GetRayPlaneIntersection(ref FPRay ray, ref FPPlane p, out FP t, out FPVector3 q)
        {
            FP denominator;
            FPVector3.Dot(ref p.Normal, ref ray.Direction, out denominator);
            if (denominator < Epsilon && denominator > -Epsilon)
            {
                //Surface of plane and line are parallel (or very close to it).
                q = new FPVector3();
            ...
            FP numerator;
            FPVector3.Dot(ref p.Normal, ref ray.Position, out numerator);
            t = (p.D - numerator) / denominator;
            //Compute the intersection position.
            FPVector3.Multiply(ref ray.Direction, t, out q);
            FPVector3.Add(ref ray.Position, ref q, out q);
            return t >= F64.C0;
        }
        ...
        /// <param name="closestPoint">Closest point on tetrahedron to point.</param>
        [Obsolete("Used for simplex tests; consider using the PairSimplex and its variants instead for simplex-related testing.")]
        public static void GetClosestPointOnTriangleToPoint(RawList<FPVector3> q, int i, int j, int k, ref FPVector3 p, RawList<int> subsimplex, RawList<FP> baryCoords, out FPVector3 closestPoint)
        {
            subsimplex.Clear();
            baryCoords.Clear();
            FP v, w;
        ...
        /// <returns>Whether or not the point is within the triangle.</returns>
        public static bool IsPointInsideTriangle(ref FPVector3 vA, ref FPVector3 vB, ref FPVector3 vC, ref FPVector3 p, FP margin)
        {
            FP u, v, w;
            GetBarycentricCoordinates(ref p, ref vA, ref vB, ref vC, out u, out v, out w);
            //Are the barycoords valid?
        ...
        public static void GetClosestPointOnSegmentToPoint(ref FPVector3 a, ref FPVector3 b, ref FPVector3 p, out FPVector3 closestPoint)
        {
            ...
            {
                FP denom = ab.X * ab.X + ab.Y * ab.Y + ab.Z * ab.Z;
                if (t >= denom)
                {
                    closestPoint = b;
                }
        ...
        /// <param name="closestPoint">Closest point on the edge to p.</param>
        [Obsolete("Used for simplex tests; consider using the PairSimplex and its variants instead for simplex-related testing.")]
        public static void GetClosestPointOnSegmentToPoint(List<FPVector3> q, int i, int j, ref FPVector3 p, List<int> subsimplex, List<FP> baryCoords, out FPVector3 closestPoint)
        {
            FPVector3 a = q[i];
            FPVector3 b = q[j];
            subsimplex.Clear();
        ...
        public static FP GetSquaredDistanceFromPointToLine(ref FPVector3 p, ref FPVector3 a, ref FPVector3 b)
        {
            FPVector3 ap, ab;
            FPVector3.Subtract(ref p, ref a, out ap);
            FPVector3.Subtract(ref b, ref a, out ab);
        ...
        public static FP GetDistancePointToPlane(ref FPVector3 point, ref FPVector3 normal, ref FPVector3 pointOnPlane)
        {
            FPVector3 offset;
        ...
        public static void GetPointProjectedOnPlane(ref FPVector3 point, ref FPVector3 normal, ref FPVector3 pointOnPlane, out FPVector3 projectedPoint)
        {
			...
			FP t = (dot - dot2) / normal.LengthSquared();
            FPVector3 multiply;
        ...
        public static void GetClosestPointOnTetrahedronToPoint(ref FPVector3 a, ref FPVector3 b, ref FPVector3 c, ref FPVector3 d, ref FPVector3 p, out FPVector3 closestPoint)
        {
            ...
            if (ArePointsOnOppositeSidesOfPlane(ref p, ref d, ref a, ref b, ref c))
            {
				...
				FP sqDist = pq.X * pq.X + pq.Y * pq.Y + pq.Z * pq.Z;
        ...
        /// <param name="closestPoint">Closest point on the tetrahedron to the point.</param>
        [Obsolete("This method was used for older GJK simplex tests.  If you need simplex tests, consider the PairSimplex class and its variants.")]
        public static void GetClosestPointOnTetrahedronToPoint(RawList<FPVector3> tetrahedron, ref FPVector3 p, RawList<int> subsimplex, RawList<FP> baryCoords, out FPVector3 closestPoint)
        {
            ...
            if (!baryCoordsFound)
            {
				...
				FP abcd = (new Matrix(tetrahedron[0].X, tetrahedron[0].Y, tetrahedron[0].Z, F64.C1,
				...
				FP pbcd = (new Matrix(p.X, p.Y, p.Z, F64.C1,
                                         tetrahedron[1].X, tetrahedron[1].Y, tetrahedron[1].Z, F64.C1,
                                         tetrahedron[2].X, tetrahedron[2].Y, tetrahedron[2].Z, F64.C1,
                                         tetrahedron[3].X, tetrahedron[3].Y, tetrahedron[3].Z, F64.C1)).Determinant();
				FP apcd = (new Matrix(tetrahedron[0].X, tetrahedron[0].Y, tetrahedron[0].Z, F64.C1,
                                         p.X, p.Y, p.Z, F64.C1,
                                         tetrahedron[2].X, tetrahedron[2].Y, tetrahedron[2].Z, F64.C1,
                                         tetrahedron[3].X, tetrahedron[3].Y, tetrahedron[3].Z, F64.C1)).Determinant();
				FP abpd = (new Matrix(tetrahedron[0].X, tetrahedron[0].Y, tetrahedron[0].Z, F64.C1,
                                         tetrahedron[1].X, tetrahedron[1].Y, tetrahedron[1].Z, F64.C1,
                                         p.X, p.Y, p.Z, F64.C1,
                                         tetrahedron[3].X, tetrahedron[3].Y, tetrahedron[3].Z, F64.C1)).Determinant();
                abcd = F64.C1 / abcd;
        ...
        public static bool RayCastSphere(ref FPRay ray, ref FPVector3 spherePosition, FP radius, FP maximumLength, out FPRayHit hit)
        {
            FPVector3 normalizedDirection;
			FP length = ray.Direction.Length();
            FPVector3.Divide(ref ray.Direction, length, out normalizedDirection);
            maximumLength *= length;
            hit = new FPRayHit();
            FPVector3 m;
            FPVector3.Subtract(ref ray.Position, ref spherePosition, out m);
			FP b = FPVector3.Dot(m, normalizedDirection);
			FP c = m.LengthSquared() - radius * radius;

            if (c > F64.C0 && b > F64.C0)
                return false;
			FP discriminant = b * b - c;
            if (discriminant < F64.C0)
                return false;

            hit.T = -b - FP.Sqrt(discriminant);
            if (hit.T < F64.C0)
                hit.T = F64.C0;
        ...
        public static void ExpandBoundingBox(ref FPBoundingBox boundingBox, ref FPVector3 sweep)
        {
            if (sweep.X > F64.C0)
        ...
        public static void GetTriangleBoundingBox(ref FPVector3 a, ref FPVector3 b, ref FPVector3 c, out FPBoundingBox aabb)
        {
#if !WINDOWS
            aabb = new FPBoundingBox();
#endif
            //X axis
            if (a.X > b.X && a.X > c.X)
            {
        ...
        public static void UpdateOrientationRK4(ref FPQuaternion q, ref Matrix3x3 localInertiaTensorInverse, ref FPVector3 angularMomentum, FP dt, out FPQuaternion newOrientation)
        {
        ...
        public static void DifferentiateQuaternion(ref FPQuaternion orientation, ref Matrix3x3 localInertiaTensorInverse, ref FPVector3 angularMomentum, out FPQuaternion orientationChange)
        {
        ...
        public static void GetBarycentricCoordinates(ref FPVector3 p, ref FPVector3 a, ref FPVector3 b, ref FPVector3 c, out FP aWeight, out FP bWeight, out FP cWeight)
        {
            FPVector3 ab, ac;
            FPVector3.Subtract(ref b, ref a, out ab);
            FPVector3.Subtract(ref c, ref a, out ac);
            ...
            FP x = triangleNormal.X < F64.C0 ? -triangleNormal.X : triangleNormal.X;
            FP y = triangleNormal.Y < F64.C0 ? -triangleNormal.Y : triangleNormal.Y;
            FP z = triangleNormal.Z < F64.C0 ? -triangleNormal.Z : triangleNormal.Z;

            FP numeratorU, numeratorV, denominator;
            if (x >= y && x >= z)
            {
            ...
            else
            {
				...
				FP distance1, distance2, distance3;
