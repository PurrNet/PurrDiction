...
namespace BEPUphysics.CollisionShapes.ConvexShapes
{
    ...
    public static class InertiaHelper
    {
        ...
        public static void GetPointContribution(FP pointWeight, ref FPVector3 center, ref FPVector3 point, out Matrix3x3 contribution)
        {
        ...
        /// </summary>
        public static FPVector3[] SampleDirections;
        ...
        public static void GenerateSphere(int subdivisionCount, out FPVector3[] vertices, out int[] triangles)
        {
            ...
            vertices = new FPVector3[GetExpectedVertexCount(subdivisionCount)];
			...
			FP length = FP.Sqrt(F64.C1 + goldenRatio * goldenRatio);
			FP x = F64.C1 / length;
			FP y = goldenRatio / length;
            vertices[0] = new FPVector3(F64.C0, x, y);
            vertices[1] = new FPVector3(F64.C0, -x, y);
            vertices[2] = new FPVector3(F64.C0, x, -y);
            vertices[3] = new FPVector3(F64.C0, -x, -y);

            vertices[4] = new FPVector3(x, y, F64.C0);
            vertices[5] = new FPVector3(-x, y, F64.C0);
            vertices[6] = new FPVector3(x, -y, F64.C0);
            vertices[7] = new FPVector3(-x, -y, F64.C0);

            vertices[8] = new FPVector3(y, F64.C0, x);
            vertices[9] = new FPVector3(-y, F64.C0, x);
            vertices[10] = new FPVector3(y, F64.C0, -x);
            vertices[11] = new FPVector3(-y, F64.C0, -x);
            ...
            for (int i = 0; i < subdivisionCount; ++i)
            {
                for (int triangleIndex = 0; triangleIndex < currentTriangleIndexCount; triangleIndex += 3)
                {
                    ...
                    if (!edges.TryGetValue(edge, out abMidIndex))
                    {
                        ...
                        //Create a vertex.
                        FPVector3 mid;
                        FPVector3.Add(ref a, ref b, out mid);
                        mid.Normalize();
                    ...
                    if (!edges.TryGetValue(edge, out bcMidIndex))
                    {
                        ...
                        //Create a vertex.
                        FPVector3 mid;
                        FPVector3.Add(ref b, ref c, out mid);
                        mid.Normalize();
                    ...
                    if (!edges.TryGetValue(edge, out acMidIndex))
                    {
                        //This edge hasn't yet been handled by another triangle.
                        FPVector3 mid;
                        FPVector3.Add(ref a, ref c, out mid);
                        mid.Normalize();
        ...
        public static void ComputeShapeDistribution(IList<FPVector3> vertices, IList<int> triangleIndices, out FP volume, out Matrix3x3 volumeDistribution)
        {
            ...
            {
                FPVector3 v2 = vertices[triangleIndices[i]];
                FPVector3 v3 = vertices[triangleIndices[i + 1]];
        ...
        /// <param name="volume">Volume of the shape.</param>
        /// <param name="volumeDistribution">Distribution of the volume as measured from the computed center.</param>
        public static void ComputeShapeDistribution(IList<FPVector3> vertices, IList<int> triangleIndices, out FPVector3 center, out FP volume, out Matrix3x3 volumeDistribution)
        {
            //Explanation for the tetrahedral integration bits: Explicit Exact Formulas for the 3-D Tetrahedron Inertia Tensor in Terms of its Vertex Coordinates
            //http://www.scipub.org/fulltext/jms2/jms2118-11.pdf
            //x1, x2, x3, x4 are origin, triangle1, triangle2, triangle3
            ...
            FPVector3 summedCenter = new FPVector3();
            FP scaledVolume = F64.C0;
            for (int i = 0; i < triangleIndices.Count; i += 3)
            {
                FPVector3 v2 = vertices[triangleIndices[i]];
                ...
                FPVector3 tetrahedronCentroid;
                FPVector3.Add(ref v2, ref v3, out tetrahedronCentroid);
                FPVector3.Add(ref tetrahedronCentroid, ref v4, out tetrahedronCentroid);
            ...
            {
                FPVector3.Multiply(ref summedCenter, F64.C0p25 / scaledVolume, out center);
                volume = scaledVolume / F64.C6;
                FP scaledDensity = F64.C1 / volume;
                FP diagonalFactor = scaledDensity / F64.C60;
                FP offFactor = -scaledDensity / F64.C120;
        ...
        public static FP ComputeMinimumRadius(IList<FPVector3> vertices, IList<int> triangleIndices, ref FPVector3 center)
        {
            ...
            for (int i = 0; i < triangleIndices.Count; i += 3)
            {
                ...
                FPVector3.Subtract(ref v4, ref v2, out v2v4);
                FPVector3 normal;
                FPVector3.Cross(ref v2v4, ref v2v3, out normal);
                ...
                if (lengthSquared > F64.C1em10)
                    FPVector3.Divide(ref normal, FP.Sqrt(lengthSquared), out normal);
                else
                    continue;

                FPVector3 fromCenterToPlane;
                FPVector3.Subtract(ref v2, ref center, out fromCenterToPlane);

                FP distance;
                FPVector3.Dot(ref normal, ref fromCenterToPlane, out distance);
                if (distance < F64.C0)
