...
namespace BEPUphysics.CollisionTests.CollisionAlgorithms.GJK
{
    ...
    public static class GJKToolbox
    {
        ...
        public static bool AreShapesIntersecting(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform transformA, ref RigidTransform transformB)
        {
            ...
            FPVector3 separatingAxis = Toolbox.ZeroVector;
        ...
        public static bool AreShapesIntersecting(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform transformA, ref RigidTransform transformB,
                                                 ref FPVector3 localSeparatingAxis)
        {
            ...
            while (count++ < MaximumGJKIterations)
            {
                ...
                FPVector3 direction;
                FPVector3.Negate(ref closestPoint, out direction);
                ...
                FP dot;
                FPVector3.Dot(ref extremePoint, ref closestPoint, out dot); //extreme point dotted against the direction pointing backwards towards the CSO. 
                if (dot > F64.C0)
                {
        ...
        public static bool GetClosestPoints(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform transformA, ref RigidTransform transformB,
                                            out FPVector3 closestPointA, out FPVector3 closestPointB)
        {
        ...
        ///<returns>Whether or not the objects were intersecting.  If they are intersecting, then the closest points cannot be identified.</returns>
        public static bool GetClosestPoints(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform transformA, ref RigidTransform transformB,
                                            ref CachedSimplex cachedSimplex, out FPVector3 closestPointA, out FPVector3 closestPointB)
        {
            RigidTransform localtransformB;
            MinkowskiToolbox.GetLocalTransform(ref transformA, ref transformB, out localtransformB);
        ...
        private static bool GetClosestPoints(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform localTransformB,
                                             ref CachedSimplex cachedSimplex, out FPVector3 localClosestPointA, out FPVector3 localClosestPointB)
        {
        ...
        public static bool RayCast(FPRay ray, ConvexShape shape, ref RigidTransform shapeTransform, FP maximumLength,
                                   out FPRayHit hit)
        {
            //Transform the ray into the object's local space.
            FPVector3.Subtract(ref ray.Position, ref shapeTransform.Position, out ray.Position);
            FPQuaternion conjugate;
            FPQuaternion.Conjugate(ref shapeTransform.Orientation, out conjugate);
            FPQuaternion.Transform(ref ray.Position, ref conjugate, out ray.Position);
            FPQuaternion.Transform(ref ray.Direction, ref conjugate, out ray.Direction);

            FPVector3 extremePointToRayOrigin, extremePoint;
            hit.T = F64.C0;
            hit.Location = ray.Position;
            hit.Normal = Toolbox.ZeroVector;
            FPVector3 closestOffset = hit.Location;

            RaySimplex simplex = new RaySimplex();

            FP vw, closestPointDotDirection;
            int count = 0;
            ...
            while (closestOffset.LengthSquared() >= Toolbox.Epsilon * simplex.GetErrorTolerance(ref ray.Position))
            {
                ...
                FPVector3.Subtract(ref hit.Location, ref extremePoint, out extremePointToRayOrigin);
                FPVector3.Dot(ref closestOffset, ref extremePointToRayOrigin, out vw);
                ...
                if (vw > F64.C0)
                {
                    ...
                    FPVector3.Dot(ref closestOffset, ref ray.Direction, out closestPointDotDirection);
            ...
            FPQuaternion.Transform(ref hit.Normal, ref shapeTransform.Orientation, out hit.Normal);
            FPQuaternion.Transform(ref hit.Location, ref shapeTransform.Orientation, out hit.Location);
            FPVector3.Add(ref hit.Location, ref shapeTransform.Position, out hit.Location);
        ...
        public static bool ConvexCast(ConvexShape sweptShape, ConvexShape target, ref FPVector3 sweep, ref RigidTransform startingSweptTransform, ref RigidTransform targetTransform,
                                  out FPRayHit hit)
        {
        ...
        public static bool ConvexCast(ConvexShape shapeA, ConvexShape shapeB, ref FPVector3 sweepA, ref FPVector3 sweepB, ref RigidTransform transformA, ref RigidTransform transformB,
                                  out FPRayHit hit)
        {
            //Put the velocity into shapeA's local space.
            FPVector3 velocityWorld;
            FPVector3.Subtract(ref sweepB, ref sweepA, out velocityWorld);
            ...
            FPQuaternion.Transform(ref velocityWorld, ref conjugateOrientationA, out rayDirection);
            ...
            do
            {
                ...
                FPVector3.Subtract(ref hit.Location, ref p, out w);
                FPVector3.Dot(ref v, ref w, out vw);
                if (vw > F64.C0)
                {
                    FPVector3.Dot(ref v, ref rayDirection, out vdir);
                    if (vdir >= F64.C0)
                    {
                        hit = new FPRayHit();
                        return false;
                    ...
                    //Shift the ray up.
                    FPVector3.Multiply(ref rayDirection, hit.T, out hit.Location);
            ...
            FPVector3.Multiply(ref velocityWorld, hit.T, out hit.Location);
        ...
        public static bool SphereCast(FPRay ray, FP radius, ConvexShape shape, ref RigidTransform shapeTransform, FP maximumLength,
                                   out FPRayHit hit)
        {
            //Transform the ray into the object's local space.
            FPVector3.Subtract(ref ray.Position, ref shapeTransform.Position, out ray.Position);
            ...
            while (v.LengthSquared() >= Toolbox.Epsilon * simplex.GetErrorTolerance(ref ray.Position))
            {
                ...
                FPVector3.Add(ref p, ref contribution, out p);

                FPVector3.Subtract(ref hit.Location, ref p, out w);
                FPVector3.Dot(ref v, ref w, out vw);
                if (vw > F64.C0)
                {
                    FPVector3.Dot(ref v, ref ray.Direction, out vdir);
                    hit.T = hit.T - vw / vdir;
                    if (vdir >= F64.C0)
                    {
        ...
        ///<param name="hit">Hit data of the sphere cast, if any.</param>
        ///<returns>Whether or not the sphere cast hit the shape.</returns>
        public static bool CCDSphereCast(FPRay ray, FP radius, ConvexShape target, ref RigidTransform shapeTransform, FP maximumLength,
                                   out FPRayHit hit)
        {
            int iterations = 0;
            while (true)
