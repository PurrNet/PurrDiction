...
namespace BEPUphysics.BroadPhaseSystems.Hierarchies
{
    ...
    public class DynamicHierarchyQueryAccelerator : IQueryAccelerator
    {
        ...
        public void GetEntries(FPBoundingBox box, IList<BroadPhaseEntry> entries)
        {
        ...
        public void GetEntries(FPBoundingSphere sphere, IList<BroadPhaseEntry> entries)
        {
            if (hierarchy.root != null)
        ...
        public bool RayCast(FPRay ray, FP maximumLength, IList<BroadPhaseEntry> entries)
        {
            if (hierarchy.root != null)
            {
        ...
        /// <param name="ray">Ray to test against the structure.</param>
        /// <param name="entries">Entries which have bounding boxes that overlap the ray.</param>
        public bool RayCast(FPRay ray, IList<BroadPhaseEntry> entries)
        {
            if (hierarchy.root != null)
            {
                hierarchy.root.GetOverlaps(ref ray, FP.MaxValue, entries);
