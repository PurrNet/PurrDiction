...
namespace Jitter2.Collision
{
    ...
    public partial class DynamicTree
    {
        ...
        /// </summary>
        public static readonly Real ExpandFactor = 0.1;

        /// <summary>
        /// Specifies a small additional expansion of the bounding box which is constant.
        /// </summary>
        public static readonly Real ExpandEps = 0.1;
        ...
        {
            if (proxy is IUpdatableBoundingBox sh) sh.UpdateWorldBoundingBox(0);
        ...
        public void Query<T>(T hits, JVector rayOrigin, JVector rayDirection) where T : ICollection<IDynamicTreeProxy>
        {
            ...
            while (_stack.Count > 0)
            {
                ...
                ref var leftNode = ref Nodes[node.Left];
                ref var rightNode = ref Nodes[node.Right];
        ...
        public void Optimize(int sweeps/* = 100*/, Real chance/* = 0.01*/, bool incremental/* = false*/)
        {
        ...
        {
            if (direction.X < 0.0) box.Min.X += direction.X;
            else box.Max.X += direction.X;

            if (direction.Y < 0.0) box.Min.Y += direction.Y;
            else box.Max.Y += direction.Y;

            if (direction.Z < 0.0) box.Min.Z += direction.Z;
        ...
        {
            var box = proxy.WorldBoundingBox;

            int parent = RemoveLeaf(proxy.NodePtr);

            int index = proxy.NodePtr;

            var pseudoRandomExt = random.NextReal();

            ExpandBoundingBox(ref box, proxy.Velocity * ExpandFactor * (1.0 + pseudoRandomExt));
        ...
        private int RemoveLeaf(int node)
        {
            ...
            while (index != NullNode)
            {
                ...
                ref var indexNode = ref Nodes[index].ExpandedBox;

                var treeBoxBefore = indexNode;
        ...
        private void InsertLeaf(int node, int where)
        {
            ...
            ref var nodeTreeBox = ref Nodes[node].ExpandedBox;
            ...
            while (!Nodes[sibling].IsLeaf)
            {
                ...
                var area = Nodes[sibling].ExpandedBox.GetSurfaceArea();
                ...
                {
                    var oldArea = Nodes[left].ExpandedBox.GetSurfaceArea();
                ...
                {
                    var oldArea = Nodes[rght].ExpandedBox.GetSurfaceArea();
