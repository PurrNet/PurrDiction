...
namespace Jitter2.Collision.Shapes
{
    ...
    {
        private static readonly Real GoldenRatio = 1.6180339887498948482045;
        ...
        private static readonly int[,] icosahedronIndices = {
        ...
        public static void MakeHull<T>(ISupportMappable support, T hullCollection, int subdivisions = 3) where T : ICollection<JTriangle>
        {
            ...
            {
                // (*)
                var v1 = icosahedronVertices[icosahedronIndices[i, 0]];
                var v2 = icosahedronVertices[icosahedronIndices[i, 1]];
                var v3 = icosahedronVertices[icosahedronIndices[i, 2]];

                support.SupportMap(v1, out var sv1);
                support.SupportMap(v2, out var sv2);
                support.SupportMap(v3, out var sv3);
            ...
            int subdivisions) where T : ICollection<JTriangle>
        {
            ...
            support.SupportMap(h1, out var sp1);
            support.SupportMap(h2, out var sp2);
            support.SupportMap(h3, out var sp3);
        ...
        {
            var oriT = JMatrix.Transpose(JMatrix.CreateFromQuaternion(orientation));

            support.SupportMap(oriT.GetColumn(0), out var res);
        ...
        public static List<JVector> SampleHull(ISupportMappable support, int subdivisions = 3)
        {
            ...
            while (stack.Count > 0)
            {
                ...
                if (depth <= 1)
                {
                    support.SupportMap(tri.V0, out var sv0);
                    support.SupportMap(tri.V1, out var sv1);
                    support.SupportMap(tri.V2, out var sv2);
                    hull.Add(sv0); hull.Add(sv1); hull.Add(sv2);
                    continue;
                }

                var ab = JVector.Normalize((tri.V0 + tri.V1) * 0.5f);
                var bc = JVector.Normalize((tri.V1 + tri.V2) * 0.5f);
                var ca = JVector.Normalize((tri.V2 + tri.V0) * 0.5f);
            ...
            out Real mass, int subdivisions = 4)
        {
            ...
            Real a = 1.0 / 60.0, b = 1.0 / 120.0;
            JMatrix canonicalInertia = new(a, b, b, b, a, b, b, b, a);

            foreach (var triangle in MakeHull(support, subdivisions))
            {
                var column0 = triangle.V0;
                var column1 = triangle.V1;
                var column2 = triangle.V2;
                ...
                var detA = transformation.Determinant();

                // now transform this canonical tetrahedron to the target tetrahedron
                // inertia by a linear transformation A
                var tetrahedronInertia = JMatrix.Multiply(transformation * canonicalInertia * JMatrix.Transpose(transformation), detA);

                var tetrahedronCom = 1.0 / 4.0 * (column0 + column1 + column2);
                var tetrahedronMass = 1.0 / 6.0 * detA;
