...
namespace Jitter2.Collision.Shapes
{
    ...
    public class TransformedShape : RigidBodyShape
    {
        ...
        {
            if (MathHelper.IsRotationMatrix(transformation, 1e-06))
            {
                JMatrix delta = transformation - JMatrix.Identity;
                type = MathHelper.UnsafeIsZero(ref delta, 1e-6) ? TransformationType.Identity : TransformationType.Rotation;
            }
            else
            {
