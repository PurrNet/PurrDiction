...
#pragma warning restore IDE0018
            ...
            if (alpha >= 0.0 && beta < 0.0)
            {
                ...
                if (gamma < 0.0 && t > 0.0)
                {
                    beta = MathR.Min(1.0, t / u.LengthSquared());
                    alpha = 1.0 - beta;
                    gamma = 0.0;
                }
                else
                {
                    gamma = MathR.Min(1.0, MathR.Max(0.0, JVector.Dot(a, v) / v.LengthSquared()));
                    alpha = 1.0 - gamma;
                    beta = 0.0;
                }
            ...
            else if (beta >= 0.0 && gamma < 0.0)
            {
                ...
                if (alpha < 0.0 && t > 0.0)
                {
                    gamma = MathR.Min(1.0, t / w.LengthSquared());
                    beta = 1.0 - gamma;
                    alpha = 0.0;
                }
                else
                {
                    alpha = MathR.Min(1.0, MathR.Max(0.0, -JVector.Dot(b, u) / u.LengthSquared()));
                    beta = 1.0 - alpha;
                    gamma = 0.0;
                }
            ...
            else if (gamma >= 0.0 && alpha < 0.0)
            {
                ...
                if (beta < 0.0 && t > 0.0)
                {
                    alpha = MathR.Min(1.0, t / v.LengthSquared());
                    gamma = 1.0 - alpha;
                    beta = 0.0;
                }
                else
                {
                    beta = MathR.Min(1.0, MathR.Max(0.0, -JVector.Dot(c, w) / w.LengthSquared()));
                    gamma = 1.0 - beta;
                    alpha = 0.0;
                }
        ...
        public void InitTetrahedron(in JVector point)
        {
            ...
            vertices[0] = new Vertex(center + 1e-2 * new JVector(MathR.Sqrt(8.0 / 9.0), 0.0, -(Real)(1.0 / 3.0)));
            vertices[1] = new Vertex(center + 1e-2 * new JVector(-MathR.Sqrt(2.0 / 9.0), MathR.Sqrt(2.0 / 3.0), -(Real)(1.0 / 3.0)));
            vertices[2] = new Vertex(center + 1e-2 * new JVector(-MathR.Sqrt(2.0 / 9.0), -MathR.Sqrt(2.0 / 3.0), -(Real)(1.0 / 3.0)));
            vertices[3] = new Vertex(center + 1e-2 * new JVector(0.0, 0.0, 1.0));
