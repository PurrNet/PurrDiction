...
namespace Jitter2.LinearMath
{
    ...
    public struct JTriangle : IEquatable<JTriangle>
    {
        ...
        [FieldOffset(0 * Real.SIZE_OF)] public JVector V0;
        [FieldOffset(3 * Real.SIZE_OF)] public JVector V1;
        [FieldOffset(6 * Real.SIZE_OF)] public JVector V2;
            ...
            out JVector normal, out Real lambda)
        {
            ...
            normal = u % v;
            Real it = 1.0 / normal.LengthSquared();

            Real denominator = JVector.Dot(direction, normal);

            if (MathR.Abs(denominator) < 1e-06)
            {
            ...
            switch (cullMode)
            {
                case CullMode.FrontFacing when lambda < 0.0:
                case CullMode.BackFacing when lambda > 0.0:
                    goto return_false;
            }
