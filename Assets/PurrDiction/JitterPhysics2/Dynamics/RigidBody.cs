...
{
    public struct FPTimeSpan
    {
        public FP64 totalSeconds;

        public static FPTimeSpan FromSeconds(FP64 seconds)
        {
            return new FPTimeSpan { totalSeconds = seconds };
        }
    }
    ...
    public struct RigidBodyData
    {
        ...
        [FieldOffset(8 + 0*Real.SIZE_OF)]
        public JVector Position;

        [FieldOffset(8 + 3*Real.SIZE_OF)]
        public JVector Velocity;

        [FieldOffset(8 + 6*Real.SIZE_OF)]
        public JVector AngularVelocity;

        [FieldOffset(8 + 9*Real.SIZE_OF)]
        public JVector DeltaVelocity;

        [FieldOffset(8 + 12*Real.SIZE_OF)]
        public JVector DeltaAngularVelocity;

        [FieldOffset(8 + 15*Real.SIZE_OF)]
        public JQuaternion Orientation;

        [FieldOffset(8 + 19*Real.SIZE_OF)]
        public JMatrix InverseInertiaWorld;

        [FieldOffset(8 + 28*Real.SIZE_OF)]
        public Real InverseMass;

        [FieldOffset(8 + 29*Real.SIZE_OF + 0)]
        public bool IsActive;

        [FieldOffset(8 + 29*Real.SIZE_OF + 1)]
        public bool IsStatic;

        [FieldOffset(8 + 29*Real.SIZE_OF + 2)]
    ...
    public sealed class RigidBody : IPartitionedSetIndex, IDebugDrawable
    {
        ...
        private Real restitution = 0.0;
        private Real friction = 0.2;
        ...
        internal Real InternalSleepTime = 0.0;
        ...
        private Real inactiveThresholdLinearSq = 0.1;
        private Real inactiveThresholdAngularSq = 0.1;
        private Real deactivationTimeThreshold = 1.0;

        private Real linearDampingMultiplier = 0.998;
        private Real angularDampingMultiplier = 0.995;

        private JMatrix inverseInertia = JMatrix.Identity;
        private Real inverseMass = 1.0;
        ...
        public Real Friction
        {
            ...
            {
                if (value < 0.0)
                    throw new ArgumentOutOfRangeException(nameof(value), "Friction must be non-negative.");
                friction = value;
            }
        }
        ...
        /// </summary>
        public FPTimeSpan DeactivationTime
        {
            get => FPTimeSpan.FromSeconds(deactivationTimeThreshold);
            set => deactivationTimeThreshold = value.totalSeconds;
        }
        ...
        public (Real angular, Real linear) DeactivationThreshold
        {
            ...
            set
            {
                if (value.linear < 0 || value.angular < 0)
                    throw new ArgumentOutOfRangeException(nameof(value), "Deactivation threshold must be non-negative.");
        ...
        {
            get => (1.0 - linearDampingMultiplier, 1.0 - angularDampingMultiplier);
            set
            {
                if (value.linear < 0 || value.linear > 1)
                    throw new ArgumentOutOfRangeException(nameof(value), "Linear damping must be between 0 and 1.");

                if (value.angular < 0 || value.angular > 1)
                    throw new ArgumentOutOfRangeException(nameof(value), "Angular damping must be between 0 and 1.");

                linearDampingMultiplier = 1.0 - value.linear;
                angularDampingMultiplier = 1.0 - value.angular;
            }
        }
        ...
        public void SetMassInertia(Real mass)
        {
            ...
            SetMassInertia();
            inverseInertia = JMatrix.Multiply(inverseInertia, 1.0 / (Data.InverseMass * mass));
            inverseMass = 1.0 / mass;
            UpdateWorldInertia();
        }
        ...
        /// </summary>
        public Real Mass => 1.0 / inverseMass;
