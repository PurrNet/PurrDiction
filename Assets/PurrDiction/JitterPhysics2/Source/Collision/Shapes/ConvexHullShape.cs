...
namespace Jitter2.Collision.Shapes
{
    ...
    public class ConvexHullShape : RigidBodyShape, ICloneableShape<ConvexHullShape>
    {
        ...
        private void Build(IReadOnlyList<JTriangle> triangles)
        {
            ...
            int len = tmpVertices.Count;
            for (int i = 0; i < len; i++)
            {
                var element = tmpVertices[i];
        ...
        public void CalculateMassInertia()
        {
            ...
            foreach (CHullTriangle tri in indices)
            {
                ...
                JVector tetrahedronCom = 1.0 / 4.0 * (column0 + column1 + column2);
                Real tetrahedronMass = 1.0 / 6.0 * detA;
            ...
            cachedCenter *= 1.0 / cachedMass;
        }

        public override void CalculateBoundingBox(in JQuaternion orientation, in JVector position, out JBoundingBox box)
        {
            JVector halfSize = 0.5 * (cachedBoundingBox.Max - cachedBoundingBox.Min);
            JVector center = 0.5 * (cachedBoundingBox.Max + cachedBoundingBox.Min);
