...
namespace Jitter2.Collision
{
    ...
    {
        private fixed long manifoldData[12*3];

        private int leftCount;
        private int rightCount;
        private int manifoldCount;

        private static readonly Real Sqrt3Over2 = 0.86602540378;
        private static readonly Real Perturbation = 0.01;

        private static readonly Real[] hexagonVertices = {
            1.0, 0.0, 0.5, Sqrt3Over2, -(Real)0.5, Sqrt3Over2,
            -(Real)1.0, 0.0, -(Real)0.5, -Sqrt3Over2, 0.5, -Sqrt3Over2
        };

        public Span<JVector> ManifoldA
        {
            get
            {
                fixed (long* ptr = manifoldData)
                    return new Span<JVector>(ptr, 6);
            }
        }

        public Span<JVector> ManifoldB
        {
            get
            {
                fixed (long* ptr = &manifoldData[18])
                    return new Span<JVector>(ptr, 6);
            }
        }
            ...
            where TA : ISupportMappable where TB : ISupportMappable
        {
            ...
            Span<JVector> mA = MemoryMarshal.CreateSpan(ref Unsafe.As<long, JVector>(ref manifoldData[0]), 6);
            Span<JVector> mB = MemoryMarshal.CreateSpan(ref Unsafe.As<long, JVector>(ref manifoldData[18]), 6);
            ...
            if (leftCount > 2)
            {
                for (int e = 0; e < rightCount; e++)
                {
                    ...
                    for (int i = 0; i < leftCount - 1; i++)
                    {
                        ...
                        sameSign = JVector.Dot(cr, cr2) > 1e-3;
                        if (!sameSign) break;
                    }

                    if (sameSign)
                    {
                        var diff = JVector.Dot(p - pA, normal);
                        mB[manifoldCount] = p;
