...
namespace Jitter2.DataStructures
{
    ...
    public readonly struct ReadOnlyPartitionedSet<T> : IEnumerable<T>
        where T : class, IPartitionedSetIndex
    {
        private readonly PartitionedSet<T> _partitionedSet;

        public ReadOnlyPartitionedSet(PartitionedSet<T> partitionedSet)
        {
            _partitionedSet = partitionedSet;
        }

        public int ActiveCount => _partitionedSet.ActiveCount;
        public int Count => _partitionedSet.Count;

        /// <summary>
        /// Returns a read-only span of all elements in the set.
        /// </summary>
        public ReadOnlySpan<T> Elements => _partitionedSet.Elements;

        /// <summary>
        /// Returns a read-only span of the active elements in the set.
        /// </summary>
        public ReadOnlySpan<T> Active => _partitionedSet.Active;

        /// <summary>
        /// Returns a read-only span of the inactive elements in the set.
        /// </summary>
        public ReadOnlySpan<T> Inactive => _partitionedSet.Inactive;

        public T this[int i] => _partitionedSet[i];

        public bool Contains(T element) => _partitionedSet.Contains(element);

        public bool IsActive(T element) => _partitionedSet.IsActive(element);

        public PartitionedSet<T>.Enumerator GetEnumerator()
        {
            return new PartitionedSet<T>.Enumerator(_partitionedSet);
        }
    ...
    public class PartitionedSet<T> : IEnumerable<T> where T : class, IPartitionedSetIndex
    {
        public struct Enumerator : IEnumerator<T>
        {
            ...
            public readonly T Current => (index >= 0 ? _partitionedSet[index] : null)!;
            ...
            {
                if (index < _partitionedSet.Count - 1)
                {
                    index++;
                    return true;
                }
