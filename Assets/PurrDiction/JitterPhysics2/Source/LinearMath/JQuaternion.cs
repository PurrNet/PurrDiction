...
namespace Jitter2.LinearMath
{
    ...
    /// </summary>
    [StructLayout(LayoutKind.Explicit, Size = 4*Real.SIZE_OF)]
    public partial struct JQuaternion : IEquatable<JQuaternion>
    {
        ...
        [FieldOffset(3*Real.SIZE_OF)] public Real W;
        ...
        public readonly JVector GetBasisX()
        {
            ...
            result.X = 1.0 - 2.0 * (Y * Y + Z * Z);
            result.Y = 2.0 * (X * Y + Z * W);
            result.Z = 2.0 * (X * Z - Y * W);
        ...
        public readonly JVector GetBasisY()
        {
            ...
            result.X = 2.0 * (X * Y - Z * W);
            result.Y = 1.0 - 2.0 * (X * X + Z * Z);
            result.Z = 2.0 * (Y * Z + X * W);
        ...
        public readonly JVector GetBasisZ()
        {
            ...
            result.X = 2.0 * (X * Z + Y * W);
            result.Y = 2.0 * (Y * Z - X * W);
            result.Z = 1.0 - 2.0 * (X * X + Y * Y);
        ...
        {
            var s = MathR.Sqrt(MathR.Max(0.0, 1.0 - quaternion.W * quaternion.W));

            Real epsilonSingularity = 1e-6;
        ...
        public static void CreateFromMatrix(in JMatrix matrix, out JQuaternion result)
        {
            ...
            if (matrix.M33 < 0)
            {
                ...
                {
                    t = 1.0 + matrix.M11 - matrix.M22 - matrix.M33;
