    ...
    public static class MathHelper
    {
        ...
        public static JQuaternion RotationQuaternion(in JVector omega, Real dt)
        {
            ...
            if (theta < 1e-3)
            {
                ...
                Real scale = 0.5 * dt - (1.0 / (Real)48.0) * dt3 * angle2;
                JVector.Multiply(omega, scale, out var axis);

                Real cos = 1.0 - (1.0/(Real)8.0) * theta * theta;

                JQuaternion res = new JQuaternion(axis.X, axis.Y, axis.Z, cos);
                Debug.Assert(MathHelper.IsZero(res.Length() - 1, 1e-2));
                return res;
            }
            else
            {
                Real halfAngleDt = 0.5 * angle * dt;
        ...
        public static bool IsRotationMatrix(in JMatrix matrix, Real epsilon/* = (Real)1e-06*/)
        {
            ...
            return MathR.Abs(matrix.Determinant() - 1.0) < epsilon;
        }
        ...
        public static JMatrix InverseSquareRoot(JMatrix m, int sweeps = 2)
        {
            ...
            for (int i = 0; i < sweeps; i++)
            {
                ...
                // M32
                if (MathR.Abs(m.M23) > 1e-6)
                {
                    phi = MathR.Atan2(1, (m.M33 - m.M22) / (2.0 * m.M23)) / 2.0;
                ...
                // M21
                if (MathR.Abs(m.M21) > 1e-6)
                {
                    phi = MathR.Atan2(1, (m.M22 - m.M11) / (2.0 * m.M21)) / 2.0;
                    (sp, cp) = MathR.SinCos(phi);
                ...
                // M31
                if (MathR.Abs(m.M31) > 1e-6)
                {
                    phi = MathR.Atan2(1, (m.M33 - m.M11) / (2.0 * m.M31)) / 2.0;
                    (sp, cp) = MathR.SinCos(phi);
            ...
            JMatrix d = new(1.0 / MathR.Sqrt(m.M11), 0, 0,
                0, 1.0 / MathR.Sqrt(m.M22), 0,
                0, 0, 1.0 / MathR.Sqrt(m.M33));
        ...
        public static JVector CreateOrthonormal(in JVector vec)
        {
            ...
            Real xa = MathR.Abs(vec.X);
            Real ya = MathR.Abs(vec.Y);
            Real za = MathR.Abs(vec.Z);
            ...
            Debug.Assert(MathR.Abs(JVector.Dot(result, vec)) < 1e-6);
